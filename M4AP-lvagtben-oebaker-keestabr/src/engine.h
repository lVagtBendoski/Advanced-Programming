...
private:
    ...
    // Outlines
    vector<unique_ptr<Shape>> outlines;
    ...
    std::chrono::time_point<std::chrono::system_clock> clockStart, clockStop;
    std::chrono::duration<double> elapsedMicroSeconds;
...
public:
    ...
    void clickButton(int indexOfButton);

    //counter for number of times clickButton is called by user
    int numPresses = 0;

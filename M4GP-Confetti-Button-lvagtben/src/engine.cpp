...
// Colors
color originalFill(1.0f, 0.0f, 0.0f, 1.0f), hoverFill(0.0f, 1.0f, 0.0f, 1.0f), pressFill(0.0f, 0.0f, 1.0f, 1.0f);
...
void Engine::initShapes() {
    ...
    spawnButton = make_unique<Rect>(shapeShader, vec2{0,height}, vec2{100, 50}, color{1, 0, 0, 1});
}
...
void Engine::processInput() {
    ...
    const float speed = 300.0 * deltaTime;
    if (keys[GLFW_KEY_UP] && screen == play) spawnButton->moveY(speed);
    if (keys[GLFW_KEY_DOWN] && screen == play) spawnButton->moveY(-speed);
    if (keys[GLFW_KEY_LEFT] && screen == play) spawnButton->moveX(-speed);
    if (keys[GLFW_KEY_RIGHT] && screen == play) spawnButton->moveX(speed);
    // TODO: Make sure the spawnButton cannot go off the screen
    if (spawnButton->getTop() > height) spawnButton->setPosY(height-spawnButton->getSize()[1]/2);
    if (spawnButton->getBottom() < 0) spawnButton->setPosY(0+spawnButton->getSize()[1]/2);
    if (spawnButton->getRight() > width) spawnButton->setPosX(width-spawnButton->getSize()[0]/2);
    if (spawnButton->getLeft() < 0) spawnButton->setPosX(0+spawnButton->getSize()[0]/2);
    ...
    if (buttonOverlapsMouse && screen == play && mousePressed) {
    ...
    if( mousePressedLastFrame && !mousePressed && screen == play && buttonOverlapsMouse) {
        spawnConfetti();
    }
